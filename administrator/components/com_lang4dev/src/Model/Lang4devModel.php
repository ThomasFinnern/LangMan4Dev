 ...
 */
class Lang4devModel extends ListModel
{
	/**
	 * Constructor.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('#__lang4dev_details', 'a'));
			...
			$subQuery = $db->getQuery(true)
				...
				->where(
					[
						...
						$db->quoteName('asso1.context') . ' = ' . $db->quote('com_lang4dev.item'),
					]
				);
			...
			$subQuery = $db->getQuery(true)
				...
				->where(
					[
						...
						$db->quoteName('type_alias') . ' = ' . $db->quote('com_lang4dev.foo'),
					]
				);
			...
			$query->join(
				...
				->where(
					[
						...
						$db->quoteName('tagmap.type_alias') . ' = ' . $db->quote('com_lang4dev.foo'),
					]
				)
